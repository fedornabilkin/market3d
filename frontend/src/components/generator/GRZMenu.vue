...
const defaultOptions = {
  ...
  keychain: new KeychainEntity({active: true}),
}
...
export default {
  ...
  methods: {
    ...
    generate() {
      ...
      try {
        if (!this.options.digits || this.options.digits.length === 0) {
          throw new Error('Не указаны цифры номера')
        }
        if (!this.options.region || this.options.region.length === 0) {
          throw new Error('Не указан код региона')
        }

        const generator = new GRZGenerator(this.options)
        const meshes = generator.generate()

        // Очищаем сцену и добавляем меши
