...
.gen-settings-panel
  ...
  .gen-settings-body
    CoasterForm(:options='options' :unit='unit')
...
const defaultOptions = {
  base: new BaseEntity({ active: true, width: 90, height: 90, depth: 3, cornerRadius: 5, shape: 'circle', segments: 64 }),
  text: new TextEntity({ active: true, message: 'подставка для кружки подружки и другана', size: 6, depth: 1, mode: 'circular', circularRadius: 35, color: '#000000' }),
  icon: new IconEntity({ active: true, name: 'user', ratio: 20 }),
  rings: new CoasterRings({ active: true, count: 4, ringWidth: 1.5, spacing: 3, startRadius: 15 }),
  code: { active: false }, // stub for Icon.vue compatibility
  keychain: new KeychainEntity({ active: false }),
};
...
export default {
  ...
  methods: {
    ...
    generate() {
      ...
      try {
        const generator = new CoasterGenerator(this.options);
