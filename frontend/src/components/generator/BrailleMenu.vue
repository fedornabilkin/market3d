...
.gen-settings-panel
  ...
  .gen-settings-body
    BrailleForm(:options='options' :unit='unit')
...
const defaultOptions = {
  base: new BaseEntity({active: true, width: 60, height: 12, depth: 3, cornerRadius: 3}),
  text: 'генератор',
  dotMode: 6,
  dotDiameter: 1.5,
  dotHeight: 1,
    ...
    generate() {
      ...
      try {
        if (!this.options.text || this.options.text.length === 0) {
          throw new Error('Не указан текст')
        }

        const generator = new BrailleGenerator(this.options)
