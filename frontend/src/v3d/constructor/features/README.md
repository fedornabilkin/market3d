...
> Правила производительности, эталонные замеры и обязательный CSG-checklist:
> [`../PERFORMANCE.md`](../PERFORMANCE.md). Изменения загрузки, recompute,
> boolean CSG и renderer нельзя сливать без этой проверки.
...
Hole-операнды и subtract-cutters обязательно раздуваются на
`CUT_INFLATE_EPS = 0.005` мм. Это предотвращает копланарные грани и
патологическое замедление `three-bvh-csg`; подробности и зафиксированный
20-кратный регресс — в [`../PERFORMANCE.md`](../PERFORMANCE.md).
